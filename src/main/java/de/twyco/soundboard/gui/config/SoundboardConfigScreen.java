...
public final class SoundboardConfigScreen extends Screen {
    ...
    public void requestFavoriteFilterRefresh() {
        refreshSoundListNextTick |= favoriteFilter != FavoriteFilter.ALL;
    }
    ...
    private int addSoundListControls(int x, int y, int width) {
        if (width >= 400) {
            int controlWidth = Math.max(1, (width - GAP * 3) / 4);
            addSortButton(x, y, controlWidth);
            addFavoriteFilterButton(x + controlWidth + GAP, y, controlWidth);
            addKeybindFilterButton(x + (controlWidth + GAP) * 2, y, controlWidth);
            addLoopFilterButton(x + (controlWidth + GAP) * 3, y, controlWidth);
    ...
    private void addFavoriteFilterButton(int x, int y, int width) {
        addRenderableWidget(
                CycleButton.builder(FavoriteFilter::getLabel, favoriteFilter)
                        .withValues(FavoriteFilter.values())
                        .create(
                                x,
                                y,
                                width,
                                BUTTON_HEIGHT,
                                Component.translatable("gui.soundboard.config.filter.favorite"),
                                (_, value) -> {
    ...
    public enum FavoriteFilter {
        ALL("gui.soundboard.config.filter.all"),
        FAVORITES("gui.soundboard.config.filter.favorite.enabled"),
        NOT_FAVORITES("gui.soundboard.config.filter.favorite.disabled");

        private final String translationKey;

        FavoriteFilter(String translationKey) {
            this.translationKey = translationKey;
        }
